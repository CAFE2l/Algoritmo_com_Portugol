...
// Lê 15 números
for (let i = 0; i < 15; i++) {
    let numero = readline.questionInt(`${cores.cinza}${estilos.negrito}Digite o ${cores.azul}${i + 1}º ${cores.cinza}número: `);
    numeros.push(numero);
}



// Array para armazenar as posições encontradas
let posicoesMultiplos10: number[] = [];

for (let i = 0; i < numeros.length; i++) {
    if (numeros[i] % 10 === 0) {
        posicoesMultiplos10.push(i);
    }
}

if (posicoesMultiplos10.length > 0) {
    console.log(`Foram encontrados múltiplos de ${cores.vermelho}10${cores.cinza} nas posições:`);
    posicoesMultiplos10.forEach(posicao => {
        console.log(`  ${cores.amarelo}Posição ${posicao}: ${cores.roxo}valor ${numeros[posicao]}${cores.cinza}`);
    });
} else {
    console.log(`${estilos.italico}${cores.vermelho}Não foram encontrados múltiplos de 10.${cores.limpa}`);
}