...
let idades: number[] = [];
while (true){
    let idade = readline.questionInt(`${cores.cinza}${estilos.negrito}{Qual da idade do aluno: `);
    idades.push(idade);
    let continuar = readline.question(`Quer continuar ${cores.vermelho}[S/N]:${cores.cinza}`).toUpperCase();
    if (continuar == "N"){
        break;
    }
}
...
console.log(`Existem ${cores.azul}${idades.length} ${cores.cinza}na turma`);
console.log(`A média de idades dos alunos da turma é de ${cores.amarelo}${media.toFixed(1)}${cores.limpa}`);
