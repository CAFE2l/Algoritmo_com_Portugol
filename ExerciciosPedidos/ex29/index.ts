...
let horas = readline.questionInt(`${cores.cinza}${estilos.negrito}Quantas horas de atividades físicas vc tem no mês? `);
let pontos = 0;
let reais = 0.05;

if (horas <= 10){
    pontos += 2;
    const pontosHora = pontos * horas;
    const total = pontosHora * reais;
    console.log(`Você ganhou ${cores.roxo}R$${total}${cores.cinza}`)
} else if (horas > 10 && horas <= 20){
    pontos += 5;
    const pontosHora = pontos * horas;
    const total = pontosHora * reais;
    console.log(`Você ganhou ${cores.amarelo}R$${total}${cores.cinza}`);
} else if (horas > 20){
    pontos += 10;
    const pontosHora = pontos * horas;
    const total = pontosHora * reais;
    console.log(`Você ganhou ${cores.verde}R$${total}${cores.cinza}`);
} else{
    console.log(`${estilos.italico}${cores.vermelho}E a esteira KKKKKKKKKKKKKKKK${cores.limpa}`);
}