...
let nota1 = readline.questionFloat(`${cores.cinza}${estilos.negrito}Digite a primeira nota do aluno: `);
let nota2 = readline.questionFloat(`Digite a segunda nota do aluno: `);
const media = (nota1 + nota2) / 2;
if(media < 5){
    console.log(`${cores.vermelho}REPROVADO${cores.limpa}`);
} else if (media > 5 && media <= 6){
    console.log(`${cores.amarelo}${estilos.negrito}RECUPERAÇÃO${cores.limpa}`);
} else {
    console.log(`${cores.verde}${estilos.negrito}APROVADO${cores.limpa}`);
}