...
while (true){
    let valor = readline.questionInt(`${cores.cinza}${estilos.negrito}Digite um valor: `);
    valores.push(valor);
...
console.log(`A média dos valores digitas é: ${cores.amarelo}${media.toFixed(1)}${cores.cinza}`);
console.log(`A soma dos valores digitados é ${cores.verde}${soma}${cores.cinza}`);
console.log(`O menor valor é ${cores.vermelho}${Math.min(...valores)}${cores.cinza}`);
console.log(`Desses números ${cores.verde}${pares}${cores.verde} são pares${cores.cinza}`);