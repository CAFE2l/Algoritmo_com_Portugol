...
for(let i=1; i <=8; i++){
    const preco = readline.questionFloat(`${cores.cinza}${estilos.negrito}Digite o preço do ${cores.azul}${i}) ${cores.cinza}produto: `);
    produtos.push(preco);
}

console.log(`${cores.cinza}O produto com maior preço foi: ${cores.verde}${Math.max(...produtos)}`);
console.log(`${cores.cinza}O produto com o menor preço foi: ${cores.vermelho}${Math.min(...produtos)}${cores.limpa}`);
