...
while (true) {
    let pergunta = readline.questionInt(`${cores.cinza}${estilos.negrito}Digite um número${cores.vermelho}[1111 para sair]:${cores.cinza} `);
    
    if (pergunta === 1111) {
        console.log(`${cores.vermelho}${estilos.negrito}Saindo do programa...${cores.cinza}`);
...
// Verifica se tem números para somar
if (numeros.length > 0) {
    let soma = numeros.reduce((total, valor) => total + valor, 0);
    
    console.log(`\n${cores.azul}=== RESULTADOS ===${cores.cinza}`);
    console.log(`Números digitados: ${cores.amarelo}${numeros}`);
    console.log(`Soma total: ${cores.ciano}${soma}${cores.cinza}`);

} else {
    console.log(`${cores.vermelho}Nenhum número foi digitado${cores.cinza}.`);
}