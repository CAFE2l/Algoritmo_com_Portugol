...
// Dados
const frase: string = "VIAGENS AUTOMOBILISTICAS COMETA";
...
let distancia = readline.questionFloat(`${cores.cinza}${estilos.negrito}Quantos Km vc vai percorrer na viagem? `);
if(distancia <= 200){
    let preco = distancia * 0.50;
    console.log(`O preço da passagem é ${cores.verde}R$${preco.toFixed(2)}${cores.limpa}`);
}else{
    let preco = distancia * 0.45;
    console.log(`${cores.cinza}${estilos.negrito}O preço da passagem é ${cores.verde}R$${preco.toFixed(2)}${cores.limpa}`);
}
