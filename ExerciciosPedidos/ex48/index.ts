...
let tothomens: number[] = [];
let totmulheres: number[] = [];

while (true){
    let salario = readline.questionFloat(`${cores.cinza}${estilos.negrito}Qual o salário do funcionário? `);
    let sexo = readline.question(`Qual o sexo do funcionário? ${cores.vermelho}[M/F]:${cores.cinza}`).toUpperCase();
    let pergunta = readline.question(`Quer Continuar? ${cores.azul}[S/N]:${cores.limpa}`).toUpperCase();

    if (sexo == "M"){
        tothomens.push(salario);
    } else if (sexo == "F"){
        totmulheres.push(salario);
    }
...
let soma = tothomens.reduce((total, valor) => total + valor, 0);
let soma2 = totmulheres.reduce((total, valor) => total + valor, 0);
console.log(`${cores.cinza}${estilos.negrito}O total pago aos homens foi de: ${cores.azul}${soma}${cores.cinza}`);
console.log(`O total pago as mulheres foi de: ${cores.roxo}${soma2}${cores.cinza}`);