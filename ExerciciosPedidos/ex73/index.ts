...
for (let i=1; i<=10; i++){
    let nota = readline.questionFloat(`${cores.cinza}${estilos.negrito}Qual a nota do ${i}) aluno: `);
    vetor.push(nota);
    let soma = vetor.reduce((valor, total) => total + valor, 0);
    let media = soma / vetor.length;
    if(nota > media){
        acimaMedia.push(nota)
    }
}
...
console.log(`A maior nota digita foi: ${cores.vermelho}${Math.max(...vetor)}${cores.cinza}`);
console.log(`A média de nota da turma é de: ${cores.amarelo}${media}${cores.cinza}`);
console.log(`A maior nota foi digita na posição ${cores.verde}${vetor.indexOf(Math.max(...vetor))}${cores.cinza}`);
console.log(`${cores.roxo}${acimaMedia.length} ${cores.cinza}alunos ficaram acima da média.`);