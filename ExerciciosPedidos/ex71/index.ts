...
let vetor: any[] = new Array(30); // Use 'any[]' se misturar tipos
let pergunta = readline.questionInt(`${cores.cinza}${estilos.negrito}Digite uma chave(1 - 15): `);
let posicoesEncontradas: number[] = []; // Array para guardar posições

for(let i = 0; i < 30; i++){
    vetor[i] = Math.floor(Math.random() * 16); // 0-15
    
    if(vetor[i] == pergunta){
        vetor[i] = `{{${vetor[i]}}}`;
        posicoesEncontradas.push(i); // Guarda a posição
    }
}

console.log(vetor);

// Mostra TODAS as posições encontradas
if(posicoesEncontradas.length > 0){
    console.log(`Valores encontrados nas posições: ${posicoesEncontradas.join(', ')}`);
    console.log(`Total encontrado: ${posicoesEncontradas.length}`);
} else {
    console.log(`Nenhum valor ${pergunta} foi encontrado.`);
}