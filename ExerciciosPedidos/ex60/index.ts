...
let soma: number[] = [];
let termo = readline.questionInt(`${cores.cinza}${estilos.negrito}Digite um termo para calcular a PA: `);
let razao = readline.questionInt(`Digite a razão da PA: `);
for (let i=termo; i<=10*razao; i+=razao){
    console.log(i);
    soma.push(i);
}

let calculo = soma.reduce((valor, total) => total + valor, 0);
console.log(`A soma dos valore é de ${cores.verde}${calculo}${cores.limpa}`);
