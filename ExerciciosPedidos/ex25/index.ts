...
let numero = Math.floor(Math.random() * 5);  // 0-4
let tentativa = readline.questionInt(`${cores.cinza}${estilos.negrito}Digite um número que o computador sorteou: `);

if (tentativa === numero) {
    console.log(`${cores.verde}Parabéns! Você acertou!${cores.cinza}`);
} else {
    // CORREÇÃO: while roda enquanto NÃO ACERTAR (!==)
    while (tentativa !== numero) {
        let novaTentativa = readline.questionInt(`${cores.vermelho}Errou! ${cores.cinza}Tente novamente: `);
        
        if (novaTentativa === numero) {
            console.log(`${cores.verde}Parabéns! Você acertou!${cores.cinza}`);
            break;  // Sai do loop
        }
        tentativa = novaTentativa;  // Atualiza pra próxima iteração
    }
}
