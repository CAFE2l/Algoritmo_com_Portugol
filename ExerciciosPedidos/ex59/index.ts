...
let mulheres:number[] = [];
let homens100:number[] = [];
let homens:number[] = [];


for (let i=1; i<=8; i++){
    let sexo = readline.question(`${cores.cinza}${estilos.negrito}Qual o seu sexo?[M/F]: `).toUpperCase();
    let peso = readline.questionFloat(`${cores.cinza}${estilos.negrito}Qual o seu peso? `);    
    if(sexo == "F"){
      mulheres.push(peso);  
    }
    if(sexo == "M" && peso >= 100){
...
let soma = mulheres.reduce((valor, total) => total + valor, 0);
let media = soma / mulheres.length;

console.log(`O total de mulheres cadastradas foi de: ${cores.vermelho}${mulheres.length}`);
console.log(`O número de homens que pesam mais de ${cores.verde}100kg${cores.cinza} foi: ${cores.verde}${homens100.length}`);
console.log(`O maior peso entre os homens foi de: ${cores.vermelho}${Math.max(...homens)}kg${cores.cinza}`);
console.log(`A média de peso entre as mulheres foi de: ${cores.amarelo}${media.toFixed(1)}${cores.cinza}`);