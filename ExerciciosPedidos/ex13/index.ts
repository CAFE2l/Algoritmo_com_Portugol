...
// Dados
const frase: string = "CALCULAR MÉDIA DO ALUNO";
...
let nota1: number, nota2: number;
nota1 = readline.questionFloat(`${cores.cinza}${estilos.negrito}Digite a primeira nota do aluno: `);
nota2 = readline.questionFloat(`Digite a segunda nota do aluno: `);
const media = (nota1 + nota2) / 2;
if(media >= 7){
    console.log(`A média do aluno foi de ${cores.verde}${media}`);
    console.log(`APROVADO${cores.limpa}`);
} else {
    console.log(`${cores.cinza}${estilos.negrito}A média do aluno foi de ${cores.vermelho}${media}`);
    console.log(`${cores.vermelho}REPROVADO${cores.limpa}`);
}
