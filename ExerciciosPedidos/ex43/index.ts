...
// Array para armazenar as idades (tipo: array de números)
let idades: number[] = [];
let mais18: number[] = [];
let menos5: number[] = [];
// Loop de 1 até 10
for (let i = 1; i <= 10; i++) {
    // questionInt já retorna um número, não precisa converter
    const pergunta = readline.questionInt(`${cores.cinza}${estilos.negrito}Digite a idade da ${cores.azul}${i}ª ${cores.cinza}Pessoa: `);
    
    // Adiciona a idade no array
    idades.push(pergunta);
    if(pergunta > 18){
        mais18.push(pergunta);
    } else if (pergunta < 5){
       menos5.push(pergunta);
    }
}
...
// Mostra todas as idades coletadas
console.log(`Idades Coletadas: ${cores.vermelho}${idades}${cores.cinza}`);
console.log(`Média das idades: ${cores.roxo}${media}${cores.cinza}`);
console.log(`A maior idade encontrada foi: ${cores.amarelo}${maior}${cores.cinza}`);
console.log(`As idades acima de 18 anos foram: ${cores.azul}${mais18}${cores.cinza}`);
console.log(`As idades com menos de 5 anos foram: ${cores.verde}${menos5}${cores.cinza}`);