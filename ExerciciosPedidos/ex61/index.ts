...
// Função que retorna os primeiros 'n' números da sequência de Fibonacci
function fibonacciSimples(n: number): number[] {
    // Array para armazenar a sequência
    const sequencia: number[] = [];
    
    // Casos base (primeiros dois números da sequência)
    if (n >= 1) sequencia.push(0);  // F(1) = 0
    if (n >= 2) sequencia.push(1);  // F(2) = 1
...
// Testando a função
console.log(`${cores.cinza}${estilos.negrito}Primeiros ${cores.azul}10${cores.cinza} números de Fibonacci:`);
console.log(fibonacciSimples(10));
