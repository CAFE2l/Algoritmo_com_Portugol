...
// Array para guardar apenas os números acima de 5
const numerosAcimaDe5 = [];

// Array para guardar apenas os números divisíveis por 3
const divisiveisPor3 = [];

// Loop que vai sortear 20 números
for (let i = 0; i < 20; i++) {
  // Math.random() gera número entre 0 e 1 (ex: 0.7834)
  // Math.random() * 11 multiplica por 11, dá entre 0 e 10.9999
  // Math.floor() arredonda pra baixo, resultado final: 0 a 10
  const numeroSorteado = Math.floor(Math.random() * 11);
  
  // Adiciona o número sorteado no array principal
  numerosSorteados.push(numeroSorteado);
  
  // Verifica se o número é maior que 5
  if (numeroSorteado > 5) {
    numerosAcimaDe5.push(numeroSorteado);
  }
  
  // Verifica se o número é divisível por 3
  // % é o operador módulo (resto da divisão)
  // Se o resto é 0, o número é divisível
  if (numeroSorteado % 3 === 0) {
    divisiveisPor3.push(numeroSorteado);
  }
}

console.log(`${cores.cinza}${estilos.negrito}Números sorteados foram: ${cores.azul}${numerosSorteados}`);
console.log(`${cores.cinza}Os números divisiveis por 3 foram: ${cores.vermelho}${divisiveisPor3}`);
console.log(`${cores.cinza}Os números acima de 5 foram: ${cores.ciano}${numerosAcimaDe5}${cores.limpa}`);