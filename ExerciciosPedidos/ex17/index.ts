...
// Dados
const frase: string = "COMPRAS NO DIA DA MULHER";
...
let sexo = readline.question(`Qual é seu sexo? ${cores.verde}[M/F]`).toUpperCase();
let compra = readline.questionFloat(`${cores.cinza}Qual o valor da sua compra? `);

if (sexo == "M"){
    const desconto = compra * 0.05;
    const total = compra - desconto;
    console.log(`${cores.azul}${nome}${cores.cinza} tem ${cores.amarelo}5%${cores.cinza} de desconto porque é homem `);
    console.log(`O valor total da compra é ${cores.verde}${total}${cores.limpa}`);
} else if (sexo == "F"){
    const desconto = compra * 0.13;
    const total = compra - desconto;
    console.log(`${cores.roxo}${estilos.negrito}${nome} ${cores.cinza}tem ${cores.amarelo}13%${cores.cinza} de desconto porque é mulher `);
    console.log(`O valor total da compra é ${cores.verde}${total}${cores.limpa}`);
}