...
let maior21: number[] = [];
while(true){
    let idade = readline.questionInt(`${cores.cinza}${estilos.negrito}Digite a sua idade:`);
    idades.push(idade);
...
let soma = idades.reduce((valor, total) => valor + total, 0);
let media = soma / idades.length;
console.log(`Foram digitadas ${idades.length} idades`);
console.log(`A média de idade foi de: ${media.toFixed(1)}`);
console.log(`tivemos ${maior21.length} pessoas acima de 21 anos`);