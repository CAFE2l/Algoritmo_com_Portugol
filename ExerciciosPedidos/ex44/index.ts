...
let totalMulheres: number = 0;
let mulheresMais20: number = 0;
let totalHomens: number = 0;

for(let i = 1; i <= 5; i++) {
    let idade = readline.questionInt(`Qual a sua idade? `);
    let sexo = readline.question(`Qual o seu sexo? [M/F] `).toUpperCase();
...
const somaIdades = idades.reduce((acc, valor) => acc + valor, 0);
const mediaGeral = somaIdades / idades.length;

// Cálculo da média dos homens (se houver homens)
let mediaHomens = 0;
if (idadesHomens.length > 0) {
    const somaIdadesHomens = idadesHomens.reduce((acc, valor) => acc + valor, 0);
    mediaHomens = somaIdadesHomens / idadesHomens.length;
}

console.log(`Mulheres com mais de 20 anos: ${mulheresMais20}`);
console.log(`Total de mulheres: ${totalMulheres}`);
console.log(`Total de homens: ${idadesHomens.length}`);
console.log(`A média de idade geral é: ${mediaGeral.toFixed(2)}`);
console.log(`A média de idade dos homens é: ${mediaHomens.toFixed(2)}`);
