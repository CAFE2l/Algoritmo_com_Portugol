...
let jogador = readline.questionInt(`${cores.cinza}${estilos.negrito}Digite um número que o computador sorteou de 1 a 10: `);
let computador = Math.floor(Math.random() * 11);
let chances = 1;

if(jogador == computador){
   console.log(`${cores.verde}${estilos.italico}Parabéns você adivinhou o número${cores.limpa}`);
} else if(jogador !== computador){
    while (chances <= 4){
        const continua = readline.questionInt(`${cores.vermelho}${estilos.negrito}${estilos.italico}ERRO! tente novamente:${cores.cinza} `);
        chances += 1;
        if (continua == computador){
            console.log(`${cores.verde}${estilos.italico} Parabéns você adivinhou o número${cores.limpa}`)
            break;
        }
