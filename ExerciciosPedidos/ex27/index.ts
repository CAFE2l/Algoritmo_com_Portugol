...
let peso = readline.questionFloat(`${cores.cinza}${estilos.negrito}Digite seu peso: `);
let altura = readline.questionFloat(`Digite sua altura: `);

const imc = peso / (altura * altura);

if (imc < 18.5){
    console.log(`${cores.vermelho}abaixo do peso${cores.cinza}`);
} else if (imc >= 18.5 && imc < 25){
    console.log(`${cores.verde}peso normal${cores.cinza}`);
} else if (imc >= 25 && imc < 30){
    console.log(`${cores.amarelo}sobrepeso${cores.cinza}`);
} else if (imc >= 30 && imc < 40){
    console.log(`${cores.roxo}obesidade${cores.cinza}`);
} else if (imc > 40){
    console.log(`${cores.vermelho}obesidade mórbida${cores.cinza}`);
} 