...
let alunos: Array<{nome: string, nota: number}> = [];

for(let i=0; i<5; i++){
    let nome = readline.question(`${cores.cinza}${estilos.negrito}Digite o nome do aluno ${cores.vermelho}${i+1}: ${cores.cinza}`);
    let nota = readline.questionFloat(`Digite a nota do ${cores.amarelo}${nome}: ${cores.cinza}`);
...
alunos.forEach(aluno => {
    console.log(`${aluno.nome} - ${cores.azul}Nota: ${aluno.nota}${cores.cinza}`);
});