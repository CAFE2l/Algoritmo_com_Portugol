...
let jogador = readline.questionInt(`${cores.cinza}${estilos.negrito}ente adivinhar um número que o computador sorteou entre 1 e 5: `);
if (jogador === comp){
   console.log(`${cores.verde}Parabéns você adivinhou${cores.cinza}`);
}

while (jogador !== comp){
    jogador = readline.questionInt(`${cores.vermelho}${estilos.negrito}ERRO! tente novamente: ${cores.cinza}`);
    if (jogador === comp){
        console.log(`${cores.verde}Parabéns você adivinhou${cores.cinza}`);
    }
}