...
let s = 0;
let n: number[] = new Array(5);
for (let i=0; i<5; i++){
    const pergunta = readline.questionInt(`${cores.cinza}${estilos.negrito}Digite um valor: `);
    n[i] = pergunta;
}

console.log(`Você digitou números pares nas posições: `);
for (let i=0; i<5; i++){
    if(i%2==0){
        console.log(n[i])
    }
...
console.log(`A soma dos valroes é ${s}`)