...
let n1 = readline.questionInt(`${cores.cinza}${estilos.negrito}Primeiro valor: `);
let n2 = readline.questionInt(`Segunda valor: `);

if (n1 > n2){
    console.log(`${cores.verde}${n1} ${cores.cinza}é maior que ${cores.vermelho}${n2}${cores.limpa}`);
}else if(n1 < n2){
    console.log(`${cores.verde}${n2} ${cores.cinza}é maior que ${cores.vermelho}${n1}${cores.limpa}`);
} else{
    console.log(`${estilos.italico}${cores.amarelo}os valores são iguais${cores.limpa}`);
}