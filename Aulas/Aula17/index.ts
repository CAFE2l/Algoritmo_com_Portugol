...
// console.log(`O maior valor foi ${Math.max(...valores)}`);
// console.log(`O menor valor foi ${Math.min(...valores)}`);
...
while (contador <= 5){
    valor = readline.questionInt(`${cores.cinza}${estilos.negrito}Digite um valor: `);
    if (contador == 1){
        maior = valor;
        menor = valor;
    } else {
