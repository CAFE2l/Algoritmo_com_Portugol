...
let idade: number;
idade = readline.questionInt(`${cores.cinza}${estilos.negrito}Qual é sua idade? `);
if (idade > 30){
    console.log(`${cores.vermelho}VELHO${cores.limpa}`)
} else{
    console.log(`${cores.verde}${estilos.negrito}JOVEM${cores.limpa}`)
}