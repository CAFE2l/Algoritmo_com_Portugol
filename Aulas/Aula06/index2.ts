...
let vel: number = readline.questionFloat(`${cores.cinza}${estilos.negrito}Qual é a velocidade do carro? `);
let multa = 0;
if (vel > 80){
    multa = (vel - 80) * 5;
    const dif = vel - 80;
    console.log(`você foi multado em${cores.vermelho}R$${multa}${cores.limpa}`);
    console.log(`Você ultrapassou ${dif}Km/h acima do permitido`);
} else{
    console.log(`${cores.verde}${estilos.italico}${estilos.negrito}Parabéns, Sua Velocidade está ótima${cores.limpa}`)
}